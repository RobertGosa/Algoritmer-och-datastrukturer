...
public class BinarySearchST<Key extends Comparable<Key>, Value> {
   ...
   /**
    ...
    public void Query(int first, int last) {
    	Key[] sortedKeys = (Key[]) new Comparable[keys.length];
    	Value[] sortedVals = (Value[]) new Object[keys.length];
    	...
    	for(int i = 0; i < keys.length; i++){
    		for(int j = i + 1; j < keys.length; j++) {
	    		if((Integer) sortedVals[j] > (Integer) sortedVals[i])  {
	    			Value tempValue = sortedVals[i];
	    			sortedVals[i] = sortedVals[j]; 
	    			sortedVals[j] = tempValue;
	    			Key keyTempValue = sortedKeys[i];
	        		sortedKeys[i] = sortedKeys[j]; 
	        		sortedKeys[j] = keyTempValue;
	    		}
    		}
    	}
    	
        for(int i = first; i <= last; i++) {
        	StdOut.print("Number: " + i + " Key: " + sortedKeys[i] + " Value: " + sortedVals[i]);
        }
    }
}